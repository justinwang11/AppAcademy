...
def title_tracks
  ...
  execute(<<-SQL)
    ...
    FROM
      tracks
    JOIN
      albums ON tracks.song = albums.title
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, AVG(counts.price)
    FROM
     (
      SELECT
        albums.price/COUNT(tracks.song) AS price, albums.asin
      FROM
        tracks
      JOIN
    ...
    JOIN
      styles ON counts.asin = styles.album
    GROUP BY
      styles.style
    HAVING
      AVG(counts.price) IS NOT NULL
    ORDER BY
      AVG(counts.price) DESC
      LIMIT 5
